...
- **Future-sprint exclusion is always on** — `buildTodoJQL` always appends `AND sprint not in futureSprints()`. `futureSprints()` requires Jira Software license; Work Management projects will get a JQL error at runtime. There is no config flag to disable this.
