...
- **`internal/analysis`, `internal/jira`, `internal/workspace`, and `internal/orchestrator` use functional-option exec injection** — `gitExecFn` and `ghExec` package-level vars removed (VC-100, VC-106, VC-107, VC-108). Patterns for tests: `NewGitParser(repoPath, analysis.WithGitRunner(fake))`, `NewPRClient(WithGHRunner(fake))`, `SetupWorkspace(ctx, cfg, id, workspace.WithGitRunner(fake))`, and `orchestrator.New(WithGitRunner(fake), WithGhRunner(fake))`. `t.Parallel()` safe in most tests. Exception: any test that captures `os.Stderr` (a global) to assert log output must remain non-parallel — e.g. `TestFetchPRReviewComments_ReviewThreadsError`. No remaining package-level func-valued exec seams in these packages.
