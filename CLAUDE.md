...
- **`internal/analysis` and `internal/jira` use functional-option exec injection** — `gitExecFn` package-level var removed (VC-100); `ghExec` package-level var removed (VC-106). `NewGitParser(repoPath, analysis.WithGitRunner(fake))` and `NewPRClient(WithGHRunner(fake))` patterns for tests; `t.Parallel()` safe in both packages. Remaining packages (`internal/orchestrator` `ghExec`/`gitExec`, `internal/workspace` `gitExecFn`/`SetGitExecFn`) still use the old global-var pattern — tracked in follow-up tickets VC-107, VC-108. Do NOT add `t.Parallel()` to tests in those packages until they are migrated.
