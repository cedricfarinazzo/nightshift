...
// emitCompression logs compression stats to the result log and emits an EventCompression.
func (o *Orchestrator) emitCompression(result *TaskResult, phase TaskStatus, s *agents.CompressStats) {
	msg := fmt.Sprintf("compress (%s): %d→%d chars (-%d%%)", phase, s.OriginalLen, s.CompressedLen, s.ReductionPct)
	o.log(result, "info", msg, map[string]any{
		"compress_original": s.OriginalLen,
		"compress_result":   s.CompressedLen,
		"compress_pct":      s.ReductionPct,
		"compress_provider": s.Provider,
		"phase":             string(phase),
	})
	o.emit(Event{
		Type:             EventCompression,
		Phase:            phase,
		CompressOriginal: s.OriginalLen,
		CompressResult:   s.CompressedLen,
		CompressPct:      s.ReductionPct,
		CompressProvider: s.Provider,
	})
}
