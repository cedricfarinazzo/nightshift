...
func (c *AnthropicClient) resolveToken(ctx context.Context) (string, error) {
	access, refresh, expiresAt, err := c.store.ReadToken()
	if err != nil {
	...
	if !expiresAt.IsZero() && time.Now().Add(expiryBuffer).After(expiresAt) {
		if refresh != "" {
			newAccess, refreshErr := RefreshToken(ctx, c.httpClient, refresh, c.store)
			if refreshErr == nil {
...
func (c *AnthropicClient) forceRefresh(ctx context.Context) (string, error) {
	_, refresh, _, err := c.store.ReadToken()
	if err != nil {
	...
	return RefreshToken(ctx, c.httpClient, refresh, c.store)
}
...
func (c *AnthropicClient) doRequest(ctx context.Context, accessToken string) (*AnthropicQuotaResponse, error) {
	...
	body, err := io.ReadAll(io.LimitReader(resp.Body, bodyLimitBytes))
	if err != nil {
